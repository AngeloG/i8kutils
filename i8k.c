...
#define I8K_VERSION		"1.15 02/06/2013"
...
static int __init i8k_probe(void)
{
	...
	if (version <= 0) {
		printk(KERN_WARNING "i8k: unable to get SMM BIOS version; using DMI version\n");
    } else {
		...
		if (strncmp(buff, bios_version, sizeof(bios_version)) != 0)
			printk(KERN_WARNING "i8k: SMM BIOS version and DMI BIOS version mismatch: %s != %s; using DMI version\n",
				buff, bios_version);
	}
